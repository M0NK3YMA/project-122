...
function askBot() {
    ...
    $("bot-response").click(function () {
        ...
        var user_bot_input_text = $("bot-response").val()

        if (user_bot_input_text != "bot-response") {
           
            //  add a new div element in the chat window
            $("bot-response").append('<div class="user__messages">' + user_bot_input_text + ' </div>')
            
            //Clear the text input box after sending message
            $("bot-response").val('bot-response');
